...
public interface ArticleRepository extends JpaRepository<Article, Long> {
  ...
  @Query(value = "SELECT new de.payload.response.ArticleDTO(a.id, a.name, a.price, d.notice)  FROM Article a  INNER JOIN a.depot d WHERE a.name=?1")
  ...
  @Query(value = "SELECT new de.payload.response.ArticleInfoDTO(e.article.id, e.article.name, e.article.price, e.article.backup,e.article.depot.notice, Sum(e.quantity) As quantity) FROM Entrance e " +
    "INNER JOIN  e.article as art " +
    "INNER JOIN  art.depot d WHERE art.name= ?1  GROUP BY art.id, e.article.depot.notice, e.article.id  ")
